...
export const PortfolioRenderer = ({
  ...
  if (isEditing && slots) {
    slots.forEach((slot) => {
      ...
      if (!slotMap[slot.clientSlotId] && !["navigation", "socials", "identity"].includes(slot.clientSlotId)) {
        slotMap[slot.clientSlotId] = (
          <div
            onClick={() => onAddExperience?.(slot.clientSlotId)}
