...
export const useHydratedLayout = (
  ...
  const { isEditing, onAddTab, onAddExperience } = useEditor();
  ...
  return useMemo(() => {
    ...
    if (manifest) {
      ...
      if (NavPattern) {
        map["navigation"] = React.createElement(NavPattern, {
          data: { ...manifest, isEditing, onAddTab }
        });
    ...
    return map;
  }, [placements, experienceLibrary, manifest, isEditing, onAddTab, onAddExperience]);
};